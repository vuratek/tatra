import {ITimelineProperties, TimelineType, ITimelineRanges, timelineController} from "./timelineController";
...
export class helper {
    ...
    public static getTimeLineProperties():ITimelineProperties {
        ...
        if (timelineController.type == TimelineType.RANGE_HOUR_MIN_TIED || timelineController.type == TimelineType.RANGE_SUBHOUR_TIED) {
            scale = 'hour';
            step = 1;
            editable = (timelineController.type == TimelineType.RANGE_SUBHOUR_TIED) ? true : false;
        }
        let zoomMax = (1000 * 60 * 60 * 24 * this.currentZoomMaxLevel);
        let zoomMin = (1000 * 60 * 60 * 24 * this.currentZoomMinLevel);
        let minDate = timelineController.minDate;
        let maxDate = timelineController.maxDate;
        return {
            scale:      scale,
            step:       step,
            editable:   editable,
    ...
    public static eventTimelineUpdate () {
        document.dispatchEvent(new CustomEvent(timelineController.EVENT_TIMELINE_UPDATED));
    }
